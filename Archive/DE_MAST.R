  ...
  for(rare.cluster in names(rare.cells.list.all) ){
    ...
    if(length(overlapGenes) > 0)
    {
      for(genei in overlapGenes)
      {
        ...
        par(mfrow=c(1,2))
        barplot(x2, main = paste(genei, rare.cluster, sep=" "), ylim=c(0,ylim), col=mycols[3], xlab="", ylab="Counts/cell", width = 0.1, axisnames = FALSE, space = 1.9);
        barplot(x1, main = "Cluster_1", ylim=c(0,ylim), width = 1, col=mycols[2], xlab="", axisnames = FALSE);
        dev.off()
      }
    }
